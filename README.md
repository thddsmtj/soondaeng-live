...
기존 순위추적 로직을 기반으로 만든 별도 사이트입니다. 기존 본사이트의 상품/키워드 데이터는 사용하지 않고, 순댕이에 새로 등록한 키워드만 추적합니다.
...
- 엑셀 `.xlsx` 리포트 다운로드
- Resend API를 통한 승인 회원별 매일 리포트 이메일 발송
- 회원 정보 수정
- 관리자 공지 작성/수정/삭제 및 본사이트 공지 댓글
...
`RESEND_API_KEY`, `REPORT_FROM`가 있으면 승인된 회원에게 각자 등록한 키워드 리포트가 발송됩니다. `REPORT_RECIPIENTS`는 관리자 사본을 추가로 받고 싶을 때만 입력하면 됩니다.
